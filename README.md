...
分析階段,原先版本 (Part 1: EDA),更新後延伸 (Part 2: ML Extension)
主要目標,透過視覺化理解薪資趨勢,建立預測模型並分析特徵關聯
資料處理,基礎清洗、GroupBy 彙整,完全編碼 (Encoding) 與 離群值過濾
核心技術,Matplotlib / Seaborn 繪圖,Scikit-learn 回歸模型、相關係數矩陣
分析深度,描述「現狀」(薪水是多少),解釋「原因」(為什麼經驗影響薪資)
主要圖表,五大基礎圖表 (長條、折線等),相關性熱力圖 (Heatmap) & 回歸趨勢圖
